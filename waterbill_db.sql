--
-- File generated with SQLiteStudio v3.1.1 on Tue Jan 1 19:03:20 2019
--
...
-- Table: BillData
CREATE TABLE BillData (mbid INTEGER PRIMARY KEY, cid INTEGER REFERENCES customer (cid), nic STRING (10), NewMeter INTEGER, Sramadhana BOOLEAN, AbsentCharge BOOLEAN, Month INTEGER NOT NULL, OldMeter INTEGER, MonthlyUsageUnit INTEGER);
...
-- Table: Customer
CREATE TABLE Customer (cid INTEGER PRIMARY KEY NOT NULL, nic STRING (10) UNIQUE NOT NULL, address_1 STRING (30), address_2 STRING (30), address_3 STRING (30), gid INTEGER, preMeter INTEGER, currentMeter INTEGER, TotalOutstandingAmount DOUBLE (5, 2), MeterNo INTEGER, lastPayment DOUBLE (5, 2), cname STRING (60));
...
CREATE TABLE MonthlyBillDetails (InvoiceNo INTEGER PRIMARY KEY, cid INTEGER REFERENCES customer (cid), nic STRING (10), "Group" STRING (20), OldMeter INTEGER, NewMeter INTEGER, MonthlyConsumption DOUBLE (5, 2), CurrentTotalAmount DOUBLE (5, 2), FixedCharge DOUBLE (4, 2), Sramadhana DOUBLE (3, 2), AbsentCharge DOUBLE (3, 2), TotalMonthlyAmount DOUBLE (5, 2), Month INTEGER NOT NULL, TotalOutstandingMonthly DOUBLE (5, 2), MonthlyUsageUnit INTEGER);

-- Table: payment
CREATE TABLE payment (pyid INTEGER PRIMARY KEY, cid INTEGER REFERENCES customer (cid), nic STRING (10) REFERENCES customer (nic), amount DOUBLE (5, 2) NOT NULL, Date DATE, oldOutStandingTotal DOUBLE (5, 2), NewOutStandingTotal DOUBLE (5, 2));
...
CREATE TRIGGER validate_mbid_before_insert_BillData BEFORE INSERT ON BillData FOR EACH ROW BEGIN SELECT CASE WHEN ((
SELECT count(mbid) 
FROM BillData
WHERE mbid = NEW.mbid)>0) 
THEN RAISE(ABORT, "Bill data Id already exist") END; END;
...
BEGIN
SELECT CASE WHEN ( (
SELECT count(pyid) FROM payment
WHERE pyid = NEW.pyid)>0) 
THEN RAISE(ABORT, "Payment id already exist") END;
END;
